...
function ArcBackdrop() {
  ...
  return (
    <svg className="pointer-events-none absolute inset-0 h-full w-full opacity-[0.18]" aria-hidden preserveAspectRatio="none">
      {arcs.map((a, i) => (
        <circle key={i} cx={a.cx} cy={a.cy} r={a.r} fill="none" stroke="white" strokeWidth={1} />
      ))}
    </svg>
  );
}

/** Frosted-glass auth card with Sign in / Register pill tabs — port of `auth/page.tsx`. */
export function AuthShell({ mode, title, description, children, footer }: AuthShellProps) {
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0a0a0a]">
      <ArcBackdrop />
      <div className="relative z-10 mx-4 w-full max-w-[360px]">
        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6 backdrop-blur-2xl">
          ...
          <button
            ...
            className="flex h-11 w-full cursor-not-allowed items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] text-sm text-white/70 transition-colors"
          >
            <GoogleIcon className="size-4" />
            Continue with Google
          </button>

          {footer && <div className="mt-5 text-center text-sm text-white/40">{footer}</div>}
        </div>
      </div>
    </div>
  );
